...
- **Git 규칙**
  ...
  3. Branch 규칙
     ...
     3. feature
        ...
        ```html
        **git checkout -b feature/login develop** /* 작업 수행 */ **git checkout develop** **git merge --no-ff feature/login** /* --no-ff option : feature branch에 존재하는 커밋 이력을 하나의 커밋
        객체로 만들어 develop 브랜치로 병합 */ **git branch -d feature/login** /* -d option: 브랜치 삭제 */ **git push origin develop**
        ```
